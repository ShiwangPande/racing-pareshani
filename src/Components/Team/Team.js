import React from 'react'
import { Link } from 'react-router-dom'

const data = [
    {
        id: 1,
        pic: "https://drive.google.com/uc?export=view&id=1fS4ityuV-qXAYb9iez6ZIQQPK4Vso7rf",
        name: 'Sarvesh Girde',
        pos: 'President',
        team: 'Transmission',

    },

    {
        id: 2,
        pic: "https://drive.google.com/uc?export=view&id=1N-zC57ejhQYycj6JSa1WEML5AWFY1Cff",
        name: 'Niraj Dudhkaware',
        pos: 'Secretary',
        team: 'Design',
    },
    {
        id: 3,
        pic: "https://drive.google.com/uc?export=view&id=1_J48F9pGQ7rxxcA7Tc0fIauGeJY4VolN",
        name: 'Sarthak Zilpe',
        pos: 'Treasurer',
        team: 'Braking',
    },
    {
        id: 4,
        pic: "https://drive.google.com/uc?export=view&id=19zMIjZ3pLKWJPDKAEyxvM67BnT1I3rJX",
        name: 'Vaibhav Pathare',
        team: 'Transmission',
    },
    {
        id: 5,
        pic: "https://drive.google.com/uc?export=view&id=18PZvWQD25blZ_iQlSuH2GQIN6vQnFJGX",
        name: 'Tanmay Turkar',
        team: 'Design',
    },
    {
        id: 6,
        pic: "https://drive.google.com/uc?export=view&id=1FU-jzsIO_FgGSzSEtgnznYh1xbQPKxxk",
        name: 'Shubham Chandurkar',
        team: 'Braking',
    },
    {
        id: 7,
        pic: "https://drive.google.com/uc?export=view&id=1GyuBY9KfH9v8pEe1nB5oShBNlJiMrQBY",
        name: 'Rutuj Kothekar',
        team: 'Steering',
    },
    {
        id: 8,
        pic: "https://drive.google.com/uc?export=view&id=1KvF1T3DxT3MeKLYxsm_GlKytgYVSt1GG",
        name: 'Om Ambarte',
        team: 'Steering',
    },
    {
        id: 9,
        pic: "https://drive.google.com/uc?export=view&id=1lZqQ_aIC1KhVEdZiguslw6lfCbiWA6aM",
        name: 'Atharva Gandhewar',
        team: 'Steering',
    },
    {
        id: 10,
        pic: "https://drive.google.com/uc?export=view&id=1S97S51MAhGVc8YTy8hqHWlvF7KzbI8J-",
        name: 'Himanshu Wanjari',
        team: 'Steering',
    },
    {
        id: 11,
        pic: "https://drive.google.com/uc?export=view&id=1Afq8lHu3grikC-R1L-Yckpz1CX3kSgJw",
        name: 'Saurav Bondhate',
        team: 'Fabrication',
    },
    {
        id: 12,
        pic: "https://drive.google.com/uc?export=view&id=12HiDkCZyxg0KyBQxVchRbX2VL9FHHmoQ",
        name: 'Kaushik Maske',
        team: 'Fabrication',
    },
    {
        id: 13,
        pic: "https://drive.google.com/uc?export=view&id=1_qZeTYTp-lRIXSQ3pLVSmORGlf0VNIJh",
        name: 'Rupesh Thengari',
        team: 'Fabrication',
    },
    {
        id: 14,
        pic: "https://drive.google.com/uc?export=view&id=1AgTbydscAtJUsrdLAM2nA2Zz7XVVHNBS",
        name: 'Vaibhav Khamankar',
        team: 'Fabrication',
    },
    {
        id: 15,
        pic: "https://drive.google.com/uc?export=view&id=19Yux0CG-HrCAUcjV3kqADQEhqSGMnj0z",
        name: 'Mallhar Yadav',
        team: 'Fabrication',
    },
    {
        id: 16,
        pic: "https://drive.google.com/uc?export=view&id=1rVY7lmE6dhaQ_y7XJRdDv0Q3qulpJGHG",
        name: 'Nikhil Pawar',
        team: 'Transmission',
    },
    {
        id: 17,
        pic: "https://drive.google.com/uc?export=view&id=19uTaGSfWoKVhpbRG8m_92SKIpwU7D7aj",
        name: 'Mahesh Deshmukh',
        team: 'Design',
    },
    {
        id: 18,
        pic: "https://drive.google.com/uc?export=view&id=1YjkUc4OoP7cnDVN6CMJAuD-liwi1Llfc",
        name: 'Anirudha Mule',
        team: 'Steering',
    },
    {
        id: 19,
        pic: "https://drive.google.com/uc?export=view&id=1cicCxcy7nJZxD0-AUB7V0_rjwltCFmG4",
        name: 'Shivani Patil',
        team: 'Design',
    },
    {
        id: 20,
        pic: "https://drive.google.com/uc?export=view&id=1za7QV5Qv7y-qgbqwBpV9fLukk_7HaJ-0",
        name: 'Renu Bhendarker',
        team: 'Design',
    },
    {
        id: 21,
        pic: "https://drive.google.com/uc?export=view&id=1jOk-l6_Mv7W4tegXlQyYvvrJbdZ3stum",
        name: 'Mohine Nakade',
        team: 'Design',
    },
    {
        id: 22,
        pic: "https://drive.google.com/uc?export=view&id=1nZIbb03JuhQCDxlihaHzUNC36ZyzGKqQ",
        name: 'Akash Paraskar',
        team: 'Design',
    },
    {
        id: 23,
        pic: "https://drive.google.com/uc?export=view&id=19rzerr_wD3B2iwtuOC6tvjBWcAKLbIBC",
        name: 'Kshitij Wandile',
        team: 'Transmission',
    },
    {
        id: 24,
        pic: "https://drive.google.com/uc?export=view&id=1A9MN9MhMVbFiQ2WEarCBuWKMB_l1lNHE",
        name: 'Rushikesh Pawade',
        team: 'Transmission',
    },
    {
        id: 24,
        pic: "https://img.icons8.com/color/350/null/gender-neutral-user.png",
        name: 'Parth Khune',
        team: 'Fabrication',
    },

    {
        id: 25,
        pic: "https://img.icons8.com/color/350/null/gender-neutral-user.png",
        name: 'Pooja Shahu',
        team: 'Design',
    },



]
function Tpage() {
    return (
        <>
            <div className='my-5'>

                <h2 className='text-center py-5 text-decoration-underline' style={{ color: "red" }}>Team</h2>


                <div className="col-sm-6 col-xl-3 mx-auto">
                    <div className="card  m-3">
                        <img className="card-img-top img-fluid " src="https://img.icons8.com/color/350/null/gender-neutral-user.png" loading="lazy" alt="" width="600" height="480" />
                        <div className="card-body text-center">
                            <h3 className="card-title h5">Prof. A. A. Uplap</h3>
                            <p className="text-secondary">Facluty Co-ordinator</p>
                        </div>
                    </div>
                </div>


                <div className="row g-5 m-1 mb-5">

                    {data.map((data) => (
                        <div className="col-sm-6 col-xl-4 mx-auto">

                            <div className="text-center">
                                <img className="card-img-top mx-auto mb-3 w-75  " src={data.pic} loading="lazy" alt=" " />
                                <div className="card-body">
                                    <h2 className="card-title h4">{data.name}</h2>
                                    <h4 className="text-secondary ">{data.pos}</h4>
                                    <h5 className="text-secondary">{data.team}</h5>
                                </div>
                            </div>
                        </div>
                    ))}
                </div>
            </div>
        </>



    )
}

export default Tpage